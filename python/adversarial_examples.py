...
def plot_nonlinear(model, im, special_name=''):
...
def l_bfgs(X, l, target, model, set, device='cpu', max_iter = 2500):
    epsilon = 0.001 # Learning rate      
    criterion = nn.CrossEntropyLoss()
    r_1 = torch.rand(1, 28, 28, requires_grad=True).to(device)   #INITIAL GUESS
    ...
    def closure():
        ...
        Yhat = model(_X_.reshape(-1,1,28,28).float().to(device))
    ...
    with torch.no_grad():           
        ...
        # plot_nonlinear(model, _X_, special_name='{}_{}'.format(PredictedClass[0].item(), l[0].item()))
    if np.random.rand() < 0.01    :
            plt.imshow((r_1).reshape(28,28).detach().numpy(), cmap='gray')
            plt.show()
            plt.close()
            # print((r_1).reshape(28,28))
            plt.imshow(X.reshape(28,28), cmap='gray')
            plt.show()
            plt.close()
            plt.imshow(_X_.reshape(28,28), cmap='gray')
            plt.show()
            plt.close()
...
def pool_gen(queue, event, data, target, model, device, set, i):
    if i%100 == 0:
        print('step', i)        
    X = data[i]     
    X = X.to(device)
    ...
    r = l_bfgs(X, l, target, model, set)             # CREATE PERTURBATION OF X.
    with torch.no_grad():            
...
def adversarial_gen(train_dataset, model, device='cpu', size= 100, target=5,
    ...
    example = torch.empty((size,dim,dim), dtype=torch.float)
    ...
    for i in np.arange(0, size,set):
        p = mp.Process(target=pool_gen, args=(done_queue, done, Data, target, model, device, set, i))
        processes.append(p)
    for p in processes:
...
def create_spicy_dataset(train_dataset, model, device='cpu', train=True, size_train=100, 
    ...
    if train:
        ...
        option_gen = {'size': size_train, 'target':target} 
        if not download:
        ...
        option_gen = {'size': size_test, 'target':target} 
        if not download:  
...
if __name__ == '__main__':
    ...
    directory = '/Users/admin/Documents/ND/ANN/project'
    ...
    if train_model:
        model = train(MyMNISTclassifier(), train_dataset, test_dataset,
                      check=False, num_epochs=20, batch_size=150).to(device)
        torch.save(model.state_dict(), join(directory, 'trained_neural_networks', 'model_dic_conv_3.pt'))
        
    else:
        model = MyMNISTclassifier().to(device)
        model.load_state_dict(torch.load(join(directory, 'trained_neural_networks', 'model_dic_conv_3.pt')))
        model.eval()
  
    # compute_misclassification(train_dataset, model)

    print('---Create adversarial examples for training and testing.---')
    directory = os.path.join(directory,'AMNIST')
    #%%
    size_train = 100
    # size_test = 100
    for target in torch.arange(10):
        create_spicy_dataset(train_dataset, model, train=True, size_train=size_train, download=False,
                              directory=directory, target=target)   
        
        # create_spicy_dataset(train_dataset, model, train=False, size_test=size_test, download=False,
        #                       directory=directory, target=target)

 #%%   
    # #JOIN ALL ADVERSARIAL TRAINING AND TEST EXAMPLES CREATED
    join_adversarial_datasets(directory=directory)
    
 #%%
    print('---Load adversarial examples for training and testing.---')
    #PHASE III: TRAIN THE PREVIOUS ANN WITH ADVERSARIAL EXAMPLES
    adversarial_train_dataset = AMNIST(directory, train=True, transform=transforms.ToTensor())
    adversarial_test_dataset = AMNIST(directory, train=False, transform=transforms.ToTensor())
#%%
    print('---Retrain the network with training adversarial examples.---')
    
    model_copy = MyMNISTclassifier()
    ...
    filtered_dataset = []
    with torch.no_grad():
        X = adversarial_train_dataset.data.reshape(-1,1,28,28).float().to(device)
        Y = model(X)
        PredictedClass = torch.argmax(Y, 1)
        filtered_dataset.append(adversarial_train_dataset.data[PredictedClass != adversarial_train_dataset.targets])
        filtered_dataset.append(adversarial_train_dataset.targets[PredictedClass != adversarial_train_dataset.targets])
    
    # compute_misclassification(filtered_dataset, model, class_amnist=False,message='filtered adversarial examples')
    torch.save(filtered_dataset,'/Users/admin/Documents/ND/ANN/project/AMNIST/filtered_dataset.pt')
    
    filtered_dataset = AMNIST(directory, train=True, training_file='filtered_dataset.pt',transform=transforms.ToTensor())
    #%%
    model_plus = train(model_copy, filtered_dataset, adversarial_test_dataset,
                  batch_size=100, num_epochs=1).to(device)
    #%%
    # compute_misclassification(train_dataset, model, message='training dataset and original model')
    #%%
    compute_misclassification(adversarial_train_dataset, model_plus, message='adversarial training dataset')
    #%%
    compute_misclassification(train_dataset, model, message='training dataset after retraining')
    compute_misclassification(train_dataset, model_plus, message='training dataset after retraining')
    #%%
    compute_misclassification(test_dataset, model, message='test dataset')
    compute_misclassification(test_dataset, model_plus, message='test dataset')
